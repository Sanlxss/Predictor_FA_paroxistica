...
def get_next_batch(dataloader, selected_features):
	...
	batch_dict["observed_data"] = batch_dict["observed_data"][:, :, selected_features]
	...
	if ("observed_mask" in data_dict) and (data_dict["observed_mask"] is not None):
		...
		batch_dict["observed_mask"] = batch_dict["observed_mask"][:, :, selected_features]

	#################################################################################
	batch_dict[ "data_to_predict"] = data_dict["data_to_predict"]
	batch_dict["tp_to_predict"] = data_dict["tp_to_predict"]

	batch_dict[ "data_to_predict"] = batch_dict[ "data_to_predict"][:, :, selected_features]
	...
	batch_dict["data_to_predict"] = batch_dict["data_to_predict"][:, :, selected_features]
	batch_dict["tp_to_predict"] = batch_dict["tp_to_predict"]
	...
	if ("mask_predicted_data" in data_dict) and (data_dict["mask_predicted_data"] is not None):
		...
		batch_dict["mask_predicted_data"] = batch_dict["mask_predicted_data"][:, :, selected_features]
...
def compute_loss_all_batches(model,
	...
	for i in range(n_batches):
		...
		batch_dict = get_next_batch(test_dataloader, reduced_features)
		...
		if args.dataset == "fibrillation":
			...
			if torch.sum(all_test_labels) != 0.:
				if args.classif_tipe_multiclass:
					...
					auc = sk.metrics.roc_auc_score(all_test_labels.cpu().numpy().reshape(-1),   
			...
			'''
			AQUI AÑADIMOS A TOTAL["LABEL PREDICTIONS"] Y TOTAL["LABELS"]
			'''
			total["label_predictions"] = classif_predictions
			total["labels"] = all_test_labels
	return total
