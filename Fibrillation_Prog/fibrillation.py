...
class Fibrillation(object):
	...
	#params = ['qrsfrontaxis', 'meanqrsdur_cross', 'transqrsmaxmag', 'qdur_I', 'qrsdur_I', 'ramp_II', 'qrsarea_aVL', 'qrsdur_V5', 'qrsppk_V6', 'qrsarea_V6']
	params=[]
	...
	def __init__(self, root, train=True, download=False, device = torch.device("cpu"), classif_tipe_multiclass=True):
		...
		self.rutaBD = '../BD_pacientes_FINAL/BD_COMPLETA/'
	...
	def download_multiclass(self, device):
		...
		total_dataset = []
		random.seed(41)
		features = [0,1, 8, 22, 30, 59, 70, 97, 220, 439, 474, 476] #COLUMNAS DE CARACTERISTICAS DEL ECG QUE SE USAN
